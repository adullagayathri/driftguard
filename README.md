...
**An end-to-end ML lifecycle that watches itself: train -> track & register (MLflow) -> serve (FastAPI) -> detect drift -> auto-retrain.** Demonstrated on a *real* distribution shift, evaluated honestly on held-out data.
...
Modules: `data` (load + year split + features) . `model` (**the only MLflow boundary** -- training + registry with aliases) . `drift` (PSI + KS, feature & concept drift) . `serving` (FastAPI) . `monitor` (the retrain loop) . `cli`.

## Run it

```bash
git clone https://github.com/adullagayathri/driftguard && cd driftguard
uv sync
...
Four modules built in parallel by separate agents against [`docs/CONTRACT.md`](docs/CONTRACT.md) (all MLflow use isolated to one module; aliases mandated over MLflow 3.x's deprecated stages). During integration, a methodology flaw was identified and addressed that originated in the contract itself -- the demo evaluated the retrained model on its own training data -- replacing it with the held-out evaluation above. Details in [`docs/REVIEW.md`](docs/REVIEW.md).

## Future Enhancements

Potential future enhancements for this project include:

-   **Terraform** to stand up the MLflow server + API on ECS/Fargate (the prompt's optional IaC).
...
This project is actively maintained by Gayathri Adulla, an AI Engineer specializing in MLOps, Generative AI, and production AI systems. With a focus on building robust and scalable machine learning solutions, Gayathri ensures the continued development and reliability of `driftguard`.
